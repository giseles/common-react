import { toUrlEncode, LooseObject } from 'common-screw'
import { history } from 'umi'

export const toPush = (pathname, data) => {
  history.push({ pathname, search: toUrlEncode(data) })
}

export const BaseReducer = {
  reducers: {
    toUpdate(state, { payload }) {
      return {
        ...state,
        ...payload
      }
...
export const toPermissionTree = (nodes) => {
  return nodes.map((n) => {
    let d = {
      key: n.id,
      title: n.name
    ...
    if (n.children) {
      d.children = toPermissionTree(n.children)
    }
    return d
  })
}
...
export const toMenuData = (data: any, configList: LooseObject = baseConfig) => {
  ...
  const toMenu = (data: any, config: any = configList, keys: any = configKeys) =>
    data.map((item: any) => {
      return keys.reduce((res: any, key: any) => {
        const realKey = config[key]
        let value = typeof realKey === 'function' ? realKey(item) : item[realKey]
        ...
        if (!!value || typeof value === 'boolean') {
          ...
          if (key === 'children') {
            ...
            const permissions = value.filter(({ isMenu }: any) => !isMenu)
            pathHasPermissionList[path] = permissions.map(({ enName, url }: any) => {
              urls[enName] = url
              return enName
            })
            breadcrumbList[path] = {
